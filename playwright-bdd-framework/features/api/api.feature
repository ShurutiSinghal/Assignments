Feature: Git Repository API Validations
    ...
    Scenario: Validate request to create a duplicate repository
        When I send a "POST" request with endpoint "/user/repos" to create a repository with the name "JMeterRepo" and description "This is a duplicate repo"
    ...
    Scenario: Validate request to create a valid repository
        When I send a "POST" request with endpoint "/user/repos" to create a repository with the name "JMeterRepoNew" and description "This is a valid repo"
        Then I should receive a response with status code 201
        And I should receive a response with status message "Created"
        And I should receive a response with body having "name" as "JMeterRepoNew"

    Scenario: Validate request to get a valid repository
        When I send a "GET" request with endpoint "/repos/Shuruti1234/JMeterRepoNew" to get the repository
        Then I should receive a response with status code 200
        And I should receive a response with status message "OK"
        And I should receive a response with body having "name" as "JMeterRepoNew"

    Scenario: Validate request to update a valid repository
        When I send a "PATCH" request with endpoint "/repos/Shuruti1234/JMeterRepoNew" to update the repository description "This is an updated repo"
        Then I should receive a response with status code 200
        And I should receive a response with status message "OK"
    ...
    Scenario: Validate request to delete repository
        When I send a "DELETE" request with endpoint "/repos/Shuruti1234/JMeterRepoNew" to delete the repository
